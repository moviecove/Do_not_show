...
ENDPOINTS:
    ...
    GET /movies/search?q=avengers  → search movies
    GET /movies/category/{cat}     → movies by category
    GET /series                    → paginated series
    GET /series/{id}               → single series
    GET /series/search?q=money     → search series
...
# Allow all origins (so your frontend can call this API freely)
app.add_middleware(
    CORSMiddleware,
    allow_origins     = ["*"],
...
def movie_to_dict(m: Movie):
    return {
        ...
        "download_links": [
            ...
            if dl.get("status") != "dead"   # only serve active links
        ],
...
def stats(db: Session = Depends(get_db)):
    ...
    nkiri_movies   = db.query(func.count(Movie.id)).filter(
        Movie.source_site == "nkiri", Movie.is_active == True).scalar()
    jarocks_movies = db.query(func.count(Movie.id)).filter(
        Movie.source_site == "9jarocks", Movie.is_active == True).scalar()
...
def movies_by_category(
    ...
    query  = db.query(Movie).filter(
        Movie.is_active == True,
        Movie.category.ilike(f"%{category}%")
    )
    total  = query.count()
    items  = query.order_by(Movie.created_at.desc()).offset((page - 1) * limit).limit(limit).all()
    return {
...
def series_by_category(
    ...
    query  = db.query(Series).filter(
        Series.is_active == True,
        Series.category.ilike(f"%{category}%")
    )
    total  = query.count()
    items  = query.order_by(Series.created_at.desc()).offset((page - 1) * limit).limit(limit).all()
    return {
...
# ── Combined search (movies + series together) ────────────────────────────────
@app.get("/search")
def search_all(
    q:     str     = Query(..., min_length=1),
    page:  int     = Query(1,  ge=1),
    ...
    results = [movie_to_dict(m) for m in movies] + [series_to_dict(s) for s in series]
